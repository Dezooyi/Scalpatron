                            ...
                            <div className="w-[60%] border-r border-border/30 p-2.5 flex flex-col gap-2">
                              ...
                              <div className={`bg-primary/5 rounded-lg border-0 shadow-lg relative overflow-hidden trade-flash-target-${selectedBot?.id} ai-flash-target-${selectedBot?.id}`}>
                                ...
                                {(() => {
                                  ...
                                  // Win rate calculation (0-100 percentage)
                                  const winRate = totalTrades > 0 ? wins / totalTrades : 0;
                                  const winRatePercentage = winRate * 100;
                                  ...
                                  // Total PnL percentage with safe null handling
                                  const totalPnlPercent = stats?.totalPnlPercent ?? 0;
                                    ...
                                    <>
                                      <div className="flex justify-between items-start p-2.5">
                                        <div>
                                          ...
                                          {/* Manual Trading Card - BUY/SELL */}
                                          <div className="mt-1 bg-primary/5 border border-primary/15 rounded-lg p-2.5">
                                            <div className="text-3xs text-primary/70 font-bold uppercase tracking-wider mb-2">
                                              Manual Trading
                                            </div>
                                            
                                            {/* Trading Mode Toggle */}
                                            <div className="mb-2.5">
                                              <label className="text-[10px] font-bold uppercase tracking-tight text-muted-foreground mb-1.5 block">
                                                Trading Mode
                                              </label>
                                            ...
                                            {selectedBot?.tradingMode === 'fixed' ? (
                                              <div className="mb-2.5 space-y-1">
                                                <label className="text-[10px] font-bold uppercase tracking-tight text-muted-foreground">
                                                  SOL per Trade <span className="text-primary font-mono">{selectedBot.tradeSize?.toFixed(2)} SOL</span>
                                                </label>
                                                <input
                                                  type="range"
                                                  min={0.01}
                                              ...
                                              <div className="mb-2.5 space-y-1">
                                                <label className="text-[10px] font-bold uppercase tracking-tight text-muted-foreground">
                                                  Max Aggressiveness <span className="text-primary font-mono">{selectedBot.aggressiveness}%</span>
                                                </label>
                                                <input
                                                  type="range"
                                                  min={1}
                                            ...
                                            <div className="grid grid-cols-2 gap-2">
                                              {/* BUY Button */}
                                              <button
                                                className="flex items-center justify-center gap-1.5 bg-green-500/20 hover:bg-green-500/30 border border-green-500/40 text-green-400 rounded-md py-2 px-3 text-sm font-bold uppercase tracking-wide transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                                                disabled={!selectedBot?.stats?.lastPrice}
                                                onClick={async () => {
                                                  if (!selectedBot?.id || !selectedBot?.stats?.lastPrice) return;
                                                  try {
                                                    const res = await fetch(`${getApiBase()}/api/bots/${selectedBot.id}/trade`, {
                                                      method: 'POST',
                                                      headers: { 'Content-Type': 'application/json' },
                                                      body: JSON.stringify({
                                                        action: 'BUY',
                                                        price: selectedBot.stats.lastPrice,
                                                      }),
                                                    });
                                                    const data = await res.json();
                                                    if (!res.ok) throw new Error(data.error || 'Trade failed');
                                                    console.log('Manual BUY executed:', data.trade);
                                                  } catch (err: any) {
                                                    console.error('Manual BUY error:', err.message);
                                                  }
                                                }}
                                              >
                                              ...
                                              {/* SELL Button */}
                                              <button
                                                className="flex items-center justify-center gap-1.5 bg-red-500/20 hover:bg-red-500/30 border border-red-500/40 text-red-400 rounded-md py-2 px-3 text-sm font-bold uppercase tracking-wide transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                                                disabled={!selectedBot?.stats?.lastPrice}
                                                onClick={async () => {
                                                  if (!selectedBot?.id || !selectedBot?.stats?.lastPrice) return;
                                                  try {
                                                    ...
                                                    const data = await res.json();
                                                    if (!res.ok) throw new Error(data.error || 'Trade failed');
                                                    console.log('Manual SELL executed:', data.trade);
                                                  } catch (err: any) {
                                        ...
                                        <div className="grid gap-4 border-t border-primary/10 px-2.5 py-3">
                                          {/* Trades & Win Rate */}
                                          <div className="flex-1 gap-3">
                                            {/* Total Trades */}
                                            <div className="text-left">
                                              <div className="text-xs-custom text-muted-foreground font-bold uppercase tracking-tight">
                                                Total Trades
                                              </div>
                                              <div className="text-xl font-black text-foreground leading-none mt-2">
                                                {totalTrades}
                                              </div>
                                            </div>
                                            {/* Win Rate */}
                                            <div className="flex-1 gap-3 mt-4 text-left">
                                              <div className="text-xs-custom text-muted-foreground font-bold uppercase tracking-tight">
                                                Win Rate
                                              </div>
                                              <div className="text-xl font-black leading-none mt-2 text-green-400">
                                          ...
                                          <div className="flex-1 gap-3 mt-4 text-left">
                                            ...
                                            <div
                                              className={`text-xl font-black mt-1 ${totalPnlPercent >= 0 ? "text-green-400" : "text-red-400"
                                                }`}
                                            >
                                              {totalPnlPercent >= 0 ? "+" : ""}
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                ...
                                <div className="bg-cyan-500/10 border border-cyan-500/20 p-3 rounded-lg flex flex-col gap-2.5 shadow-md justify-between">
                                  <div className="flex items-center justify-between gap-2">
                                    <div className="text-l font-bold text-cyan-300 flex items-center gap-1.5 shrink-0">
