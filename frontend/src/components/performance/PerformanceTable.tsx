...
function PerformanceTableBase({ bots, perBot, tokenSymbol, onSelectBot, selectedBotId }: PerformanceTableProps) {
  ...
  if (rows.length === 0) {
    return (
      <div className="flex items-center justify-center h-24 text-[11px] text-zinc-500">
  ...
  return (
    <div className="rounded-md border border-white/10 dark:border-white/5 overflow-hidden">
      <Table>
        <TableHeader>
            ...
            {COLUMNS.map((col) => {
              ...
              return (
                <TableHead
                  ...
                  className={`cursor-pointer select-none text-[9px] uppercase tracking-wider font-bold text-zinc-500 hover:text-zinc-300 whitespace-nowrap ${
          ...
          {sortedRows.map((row) => {
            ...
            return (
              <TableRow
                ...
                className={`cursor-pointer border-white/5 text-[11px] tabular-nums transition-colors ${
              ...
              >
                <TableCell className="font-bold whitespace-nowrap">
                  <div className="flex items-center gap-1.5 min-w-[120px]">
                    ...
                    <span className="text-zinc-400 dark:text-zinc-300 truncate max-w-[120px]">{row.name}</span>
                    <span
                    ...
                    {row.streakLabel !== "—" && (
                      <span className="text-[8px] font-mono text-zinc-500 shrink-0">{row.streakLabel}</span>
                    )}
                  </div>
                </TableCell>
                <TableCell className={`text-right font-black ${pnlPositive ? "text-emerald-400" : "text-red-400"}`}>
                  {formatPct(row.netPnlPercent)}
                </TableCell>
                <TableCell className={`text-right ${row.winRate >= 50 ? "text-emerald-400" : "text-zinc-300"}`}>
                  {row.winRate.toFixed(0)}%
                </TableCell>
                <TableCell className={`text-right ${row.profitFactor >= 1 ? "text-emerald-400" : "text-red-400"}`}>
                  {formatRatio(row.profitFactor)}
                </TableCell>
                <TableCell className="text-right text-zinc-300">{row.closedTrades}</TableCell>
                <TableCell className="text-right text-red-400">-{row.maxDrawdown.toFixed(1)}%</TableCell>
                <TableCell className="text-right text-zinc-400 whitespace-nowrap">{formatDuration(row.avgHoldMs)}</TableCell>
                <TableCell className={`text-right ${row.expectancy >= 0 ? "text-emerald-400" : "text-red-400"}`}>
